...
CREATE TABLE IF NOT EXISTS flow_stations (
...
CREATE INDEX IF NOT EXISTS idx_stations_river ON flow_stations(river_name);
CREATE INDEX IF NOT EXISTS idx_stations_name ON flow_stations(station_name);
...
CREATE TABLE IF NOT EXISTS grdc_daily_flow (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (station_id) REFERENCES flow_stations(station_id) ON DELETE CASCADE,
